...
class FakeFlickr(AbstractDataset):
    """
    FakeFlickr inpainting dataset.

    directory structure (relative to dataset_paths['FF']):
    fake-flickr-inpaintings/{model}/img/{id}.png   <- tampered images
    masks_entities_ensemble/{model}_dilated/masks/mask_{id}.png  <- binary masks

    List file format (CSV): image_rel_path,mask_rel_path
    """
    ...
    def __init__(self, crop_size, grid_crop, img_list: str, max_dim=None, aug=None):
        ...
        self._root_path = dataset_paths['FF']
